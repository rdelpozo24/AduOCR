...
import MfaLogin from './components/MfaLogin.tsx';
...
import { AuthState, ProcessedDocument, AppTab } from './types.ts';

const App: React.FC = () => {
  const [auth, setAuth] = useState<AuthState>({
    isAuthenticated: false, // Forzado a false para que pase por el login previo solicitado
    user: null,
    mfaVerified: false,
  });
  ...
  const renderContent = () => {
    switch (activeTab) {
            ...
            <div className="lg:col-span-7 space-y-6">
              <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-xl p-8 text-white shadow-lg relative overflow-hidden">
                <div className="relative z-10">
                  <h2 className="text-2xl font-bold mb-2">Redistribución Automática</h2>
                  <p className="text-blue-100 text-sm opacity-90 leading-relaxed max-w-md">
                    DocuMind utiliza IA para clasificar y renombrar tus documentos. Los archivos detectados se normalizan según su número de expediente o ID.
                  </p>
                  <button 
                  ...
                  >
                    <i className="fas fa-route"></i> Gestionar Reglas
                  </button>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                  <div className="text-gray-400 text-xs font-bold uppercase mb-2">Procesados</div>
                  <div className="text-3xl font-bold text-gray-800">{processedDocs.length}</div>
                </div>
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                  <div className="text-gray-400 text-xs font-bold uppercase mb-2">Destinos Activos</div>
                  <div className="text-3xl font-bold text-gray-800">4</div>
                </div>
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col justify-between">
                  <div className="text-gray-400 text-xs font-bold uppercase">Seguridad</div>
                  <div className="text-xs font-bold text-green-600 flex items-center gap-1">
                    <i className="fas fa-shield-alt"></i> MFA Activo
                  </div>
                </div>
              </div>
            </div>
      ...
      default:
        return <div>Tab no implementada.</div>;
    }
  };
      ...
      <aside className={`bg-slate-900 text-white transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col shrink-0`}>
        ...
        <nav className="flex-1 mt-6 px-4 space-y-2">
          {[
            ...
            { id: 'redistribution', icon: 'fa-route', label: 'Rutas' },
          ].map(item => (
            <button 
              key={item.id} 
